...
public class Ruleta {
	...
	//TODO: En lugar de avanzar aleatoriamente, avanzan a la par pero con 3 cantidades de veces diferente
	...
	public Puntuacion getPuntuacion() {
		...
		if (m[0][0].equals(m[1][1]) && m[1][1].equals(m[2][2])) {
			return new Puntuacion("diagonal descendente [\\]" + (m[0][0].getValor() > 1 ? " de " + m[0][0].toString() : ""),
					2.5 * m[0][0].getValor());
		}
		if (m[2][0].equals(m[1][1]) && m[1][1].equals(m[0][2])) {
			return new Puntuacion("diagonal ascendente [/]" + (m[2][0].getValor() > 1 ? " de " + m[2][0].toString() : ""),
					2.5 * m[2][0].getValor());
		}
